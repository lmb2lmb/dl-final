...
def Attention_Matrix(K, Q, use_mask=False):
	...
	scores = tf.matmul(Q, tf.transpose(K, perm=[0, 2, 1])) / math.sqrt(K.get_shape()[2])
	sftmax = None
	if use_mask:
		sftmax = tf.nn.softmax(scores + atten_mask)
	else:
		sftmax = tf.nn.softmax(scores)
...
class Atten_Head(tf.keras.layers.Layer):
	def __init__(self, input_size, output_size, use_mask):		
		...
		self.K_weight = self.add_weight(name='K', shape=[input_size, output_size], trainable=True)
		self.V_weight = self.add_weight(name='V', shape=[input_size, output_size], trainable=True)
		self.Q_weight = self.add_weight(name='Q', shape=[input_size, output_size], trainable=True)
